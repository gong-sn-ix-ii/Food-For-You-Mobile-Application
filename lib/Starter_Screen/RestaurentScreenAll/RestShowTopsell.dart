...
import '../ClientScreenAll/ClientScreen.dart';

class RestShowReview extends StatefulWidget {
  const RestShowReview({Key? key}) : super(key: key);

  @override
  State<RestShowReview> createState() => _RestShowReviewState();
}

class _RestShowReviewState extends State<RestShowReview> {
  @override
  Database db = Database.instance;
  Color? enableButton;
  Future<int> Length() async {
    List<FoodModel> size = await db.getAllFoodMenu_Database();
  ...
  Future<List<FoodModel>> Database_getAllFoodModelDelayedSetTimer(int timer) {
    return Future.delayed(
        Duration(seconds: timer), () => db.getAllFoodMenu_Database());
  }
  ...
  void initState() {
    Future.delayed(Duration.zero, () async {
      int x = await Length();
      foodList = await db.getAllFoodMenu_Database();
      for (var i = 1; i < x; i++) {
        print("########################### initState()");
        check.add(false);
      }
                      ...
                      onPressed: (_isEnableActiveButton != null)
                          ? () async {
                              ...
                              showDialog(
                                  context: context,
                                  builder: (contextMumi) {
                                    dialogContext = contextMumi;
                                    return Waiting_Screen();
                                  });
                              print("click\n going to maekFoodmenu");
                              setState(() {});
                              print("######====> ${savingInformation}");
                              String masterID = await db.my_uid();
                              List<FoodModel> foodmenu = [];
                              for (var i = 0;
                                  i < savingInformation!.length;
                                  i++) {
                                foodmenu.addAll(await db.Search_FoodmenuForData(
                                    search: savingInformation![i],
                                    masterID: masterID));
                              }
                              for (var i = 0; i < foodmenu.length; i++) {
                                print(
                                    "!!!!!!--------------> Foodmenu Searching |: ${i + 1}.${await foodmenu[i].food_name} Length ${await foodmenu.length}");
                              }
                              int loop = 0;
                              String uid = await db.my_uid();
                              List<Map<String, dynamic>> foodset = [];
                              for (var i = 0; i < foodmenu.length; i++) {
                                foodset.addAll([
                                  {
                                    "name": foodmenu[i].food_name,
                                    "type": foodmenu[i].food_type,
                                    "price": foodmenu[i].food_price,
                                    "image": foodmenu[i].food_image,
                                    "linkImage": foodmenu[i].food_linkImage,
                                  }
                                ]);
                              }
                              MakeListModel makeListModel =
                                  MakeListModel(makelist_order: foodset);
                              List<MakeListModel> makelistLength =
                                  await db.get_MakeListAll();
                              int id = makelistLength.length;
                              print("!!!!!!!!-----------> get ID ==> ${id}");
                              await db.add_MakeList(makeListModel, id: id);
                              String my_uid = await db.my_uid();
                              Navigator.pop(dialogContext!);
                              await db.update_Database(
                                  collection_start: "user",
                                  collection_end: "makeList",
                                  user_ID: "defualt${id}",
                                  field: "id",
                                  value: id.toString(),
                                  typeString: false);
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => MakeFoodList(id)));
                              //ข้อมูลมาไม่ครบ ค้าง ต้องการ ให้ savingInformation ถูกเพิ่มเข้าไปใน Database
                              //แล้ว หน้าเพิ่มจำนวนข้อมูลให้ ForEach ดึงมาจาก Database เลย
                            }
                          : null,
                      style: ElevatedButton.styleFrom(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              "ทำรายการ", //ทำรายการ [${check_length}]
                              style: TextStyle(
